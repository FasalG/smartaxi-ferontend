...
@Component({
    ...
    template: `
    <div class="position-fixed" style="top: 20px; right: 20px; z-index: 1050;" *ngIf="showBell()">
      <div class="position-relative">
        <button class="btn btn-light bg-white rounded-circle shadow-lg p-2 position-relative border transition-all hover-scale" 
          ...
          <svg width="26" height="26" fill="none" stroke="currentColor" viewBox="0 0 24 24" class="text-navy">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
          </svg>
          <span *ngIf="overdueReceivables().length > 0" 
                class="position-absolute align-items-center justify-content-center d-flex bg-danger text-white rounded-circle fw-bold shadow-sm" 
        ...
        <!-- Dropdown / Overlay -->
        <div *ngIf="isOpen()" 
             class="position-absolute end-0 mt-3 bg-white rounded-4 shadow-lg border overflow-hidden animate-fade-in" 
             style="width: 340px; right: 0;">
          <div class="p-3 bg-light border-bottom d-flex justify-content-between align-items-center">
            <h6 class="fw-bold text-navy mb-0">Credit Alerts</h6>
            <span class="badge bg-danger rounded-pill">{{ overdueReceivables().length }} Overdue</span>
          </div>
